...
Puedes etiquetar con **CSV de píxeles** o con un **vector espacial** (GeoJSON / Shapefile).

### Opción A — CSV (coordenadas de píxel)
...
### Opción B — GeoJSON / Shapefile

Polígonos o puntos con atributo de clase (default `unit`). Idealmente en el **mismo CRS** que el GeoTIFF del producto (export desde QGIS sobre el mapa). Si el `.prj` / CRS del vector difiere del cubo, se reproyecta.

Plantilla: `examples/training_polygons.example.geojson`

```bash
python -m crism_pipeline classify \
  --input data/raw/escena \
  --method supervised \
  --training examples/mi_entrenamiento.geojson

# Si el campo de clase no se llama "unit":
python -m crism_pipeline classify \
  --input data/raw/escena \
  --method supervised \
  --training zonas.shp \
...
`--training-csv` sigue aceptándose como alias de `--training`.

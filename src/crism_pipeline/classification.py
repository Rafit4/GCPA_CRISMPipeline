...
from .spatial import VECTOR_SUFFIXES, pixels_under_vector

_CSV_SUFFIXES = {".csv"}
_VECTOR_SUFFIXES = VECTOR_SUFFIXES
...
def training_df_from_vector(
    cube: SRCube,
    path: Path,
    *,
    unit_field: str = "unit",
    max_pixels_per_unit: int | None = 5000,
    random_state: int = 42,
) -> pd.DataFrame:
    """
    Rasteriza puntos/polígonos etiquetados sobre la grilla del cubo SR.

    El archivo debe tener un atributo de clase (default: ``unit``).
    """
    pix = pixels_under_vector(
        cube.map_info,
        cube.data.shape[:2],
        path,
        valid_mask=cube.valid_mask,
        label_field=unit_field,
        require_label=True,
        max_pixels_per_feature=max_pixels_per_unit,
        random_state=random_state,
    )
    # Un píxel puede aparecer en varias features; nos quedamos con la primera.
    pix = pix.drop_duplicates(subset=["line", "sample"], keep="first")
    return pd.DataFrame(
        {
            "row": pix["line"].astype(int),
            "col": pix["sample"].astype(int),
            "unit": pix["label"].astype(str),
        }
    )
...
def load_training_dataframe(
    ...
    *,
    ...
    """Carga entrenamiento desde CSV (row,col,unit) o vector espacial."""
    path = Path(path)
    if not path.is_file():
    ...
    if suffix in _VECTOR_SUFFIXES:
        ...
        return training_df_from_vector(
            cube,
            path,
    ...
    raise ValueError(
        f"Formato de entrenamiento no soportado: {suffix}. "
        f"Usa CSV {_CSV_SUFFIXES} o vector {_VECTOR_SUFFIXES}."
    )
...
def classify_supervised(
    ...
    *,
    ...
    """
    Clasificación supervisada con CSV (row,col,unit) o vector
    (GeoJSON / Shapefile) con atributo de clase.
    """
    cfg = pipeline_config()["classification"]
    feature_names = feature_names or cfg["default_features"]
    df = load_training_dataframe(cube, training_path, unit_field=unit_field)

    features = _feature_stack(cube, feature_names)
    rows_i = df["row"].astype(int).to_numpy()
    cols_i = df["col"].astype(int).to_numpy()
    ...
    if (
        ...
        raise ValueError(
            f"Hay coordenadas fuera del cubo ({h}×{w}). Revisa row/col o el vector."
        )

    X = features[rows_i, cols_i, :]
    y = df["unit"].astype(str)

    stratify = y if y.value_counts().min() >= 2 else None
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.25, random_state=cfg["random_state"], stratify=stratify
    )
    scaler = StandardScaler()
...
def run_classification_pipeline(
    ...
    *,
    ...
    training = training_path or training_csv
    ...
    if method == "kmeans":
        ...
        if not training:
            raise ValueError(
                "method=supervised requiere --training (CSV, GeoJSON o Shapefile)"
            )
        labels, meta, report = classify_supervised(
            cube, training, unit_field=unit_field
        )

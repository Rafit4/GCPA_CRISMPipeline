...
COLORS = {
    ...
    "warn": "#e0b35a",
    "input": "#1a2740",
    "input_disabled": "#0c121c",
    "text_disabled": "#4d5f78",
    "border_disabled": "#182233",
    "btn_disabled": "#121a28",
}
...
def _logo_path() -> Path:
    ...
    for folder in (package_assets, root_assets):
        for name in ("gcpa_logo.png", "gcpa_logo.jpeg", "gcpa_logo.jpg"):
            candidate = folder / name
            if candidate.is_file():
                return candidate
    return package_assets / "gcpa_logo.png"
...
def set_entry_enabled(entry: ctk.CTkEntry, enabled: bool) -> None:
    """Activa/desactiva un entry con colores distintivos de estado."""
    if enabled:
        entry.configure(
            state="normal",
        ...
        entry.configure(
            state="disabled",
            fg_color=COLORS["input_disabled"],
            text_color=COLORS["text_disabled"],
            border_color=COLORS["border_disabled"],
        )


def set_button_enabled(button: ctk.CTkButton, enabled: bool) -> None:
    if enabled:
        button.configure(state="normal", fg_color=COLORS["accent_dim"], text_color=COLORS["text"])
    else:
        button.configure(
            state="disabled",
...
class Worker:
    ...
    def request_cancel(self) -> None:
        if not self._busy:
            return
        self._cancel.set()
        self._q.put(("log", "⏹ Cancelando…\n"))
        self._q.put(("progress", 0.0, "Cancelando…"))
        ...
        def target() -> None:
            try:
                ...
                if self._cancel.is_set():
                    self._q.put(("cancel", f"⏹ Cancelado: {label}"))
                else:
                ...
                if isinstance(exc, DownloadCancelled) or self._cancel.is_set():
                    self._q.put(("cancel", f"⏹ Cancelado: {label}"))
                else:
        ...
        try:
            while True:
                ...
                if kind == "log":
                    ...
                    self._cancel.clear()
                    app.append_log(item[1] + "\n")
                    app.set_progress(0.0, "Cancelado")
...
class PathRow(ctk.CTkFrame):
    def __init__(
        ...
        *,
        ...
        browse_btn = ctk.CTkButton(self, text="…", width=36, command=self._browse)
        browse_btn.pack(side="left")
        self._controls.append(browse_btn)
        if show_raw:
            raw_btn = ctk.CTkButton(self, text="Raw", width=52, command=self._pick_raw)
            raw_btn.pack(side="left", padx=(6, 0))
            self._controls.append(raw_btn)
        if show_if:
    ...
    def set_enabled(self, enabled: bool) -> None:
        for w in self._controls:
            if isinstance(w, ctk.CTkEntry):
                set_entry_enabled(w, enabled)
            elif isinstance(w, ctk.CTkButton):
                set_button_enabled(w, enabled)
            else:
...
class CrismApp(ctk.CTk):
    ...
    def _tab_body(self, name: str) -> ctk.CTkScrollableFrame:
        """Contenedor con scroll vertical para el contenido de una pestaña."""
    ...
    def _build_download(self) -> None:
        ...
        for val, text in (
            ...
            ctk.CTkRadioButton(
                modes, text=text, variable=self.dl_mode, value=val, command=self._on_dl_mode_change
        ...
        self.dl_pdsid_entry = make_entry(row, textvariable=self.dl_pdsid)
        self.dl_pdsid_entry.pack(side="left", fill="x", expand=True)
        ...
        self.dl_bbox_entries: list = []
        for v in self.dl_bbox:
            entry = make_entry(bbox, textvariable=v, width=90)
            entry.pack(side="left", padx=3)
            self.dl_bbox_entries.append(entry)
        ...
        self.dl_max_entry = make_entry(row2, textvariable=self.dl_max, width=90)
        self.dl_max_entry.pack(side="left")
        ...
        self.dl_cancel_btn = ctk.CTkButton(
            ...
            hover_color=COLORS["err"],
            command=self._cancel_download,
            state="disabled",
        )
        self.dl_cancel_btn.pack(side="left", padx=(0, 8))
        self.dl_btn = ctk.CTkButton(
            btns, text="Descargar desde ODE", height=36, command=self._run_download
        )
        self.dl_btn.pack(side="left")
        self._update_dl_inputs()

    def _on_dl_mode_change(self) -> None:
        self._update_dl_inputs()
        self._sync_dl_cli()

    def _update_dl_inputs(self) -> None:
        mode = self.dl_mode.get()
        self.dl_ids_file.set_enabled(mode == "ids_file")
        set_entry_enabled(self.dl_pdsid_entry, mode == "pdsid")
        for entry in self.dl_bbox_entries:
            set_entry_enabled(entry, mode == "bbox")
    ...
    def _run_download(self) -> None:
        ...
        def job() -> None:
            ...
            if mode == "ids_file":
                ...
                to_download = ids[:max_p] if max_p else ids
                self.worker.log(
                    f"IDs leídos ({len(ids)}"
                    + (f", limitados a {len(to_download)}" if max_p else "")
                    + f"): {', '.join(to_download)}\n"
                )
    ...
    def _build_classify(self) -> None:
        t = self._tab_body("Clasificación")
        ...
        ctk.CTkOptionMenu(
            ...
            command=lambda _: self._on_cl_method_change(),
        ).pack(side="left")
        ...
        self.cl_n_entry = make_entry(row2, textvariable=self.cl_n, width=80)
        self.cl_n_entry.pack(side="left")
        self.cl_n.trace_add("write", lambda *_: self._sync_classify_cli())

        self.cl_training = PathRow(
            t,
            "Entrenamiento",
        ...
        self.cl_training.pack(fill="x", pady=6)
        ctk.CTkLabel(
            t,
            text="Supervised: CSV (row,col,unit), GeoJSON o Shapefile con atributo 'unit'.",
            text_color=COLORS["muted"],
            wraplength=720,
            justify="left",
        ).pack(anchor="w", pady=(0, 4))
        ctk.CTkButton(t, text="Clasificar", height=36, command=self._run_classify).pack(anchor="e", pady=10)
        self._update_cl_inputs()

    def _on_cl_method_change(self) -> None:
        self._update_cl_inputs()
        self._sync_classify_cli()

    def _update_cl_inputs(self) -> None:
        method = self.cl_method.get()
        set_entry_enabled(self.cl_n_entry, method == "kmeans")
        self.cl_training.set_enabled(method == "supervised")
    ...
    def _sync_classify_cli(self) -> None:
        ...
        if self.cl_method.get() == "supervised":
            parts += ["--training", _q(self.cl_training.var.get())]
        self.cli_var.set(" ".join(parts))
    ...
    def _run_classify(self) -> None:
        ...
        training = self.cl_training.get() if method == "supervised" else None
        if method == "supervised" and (not training or not training.is_file()):
            messagebox.showerror(
                "Clasificación",
                "Indica un archivo de entrenamiento válido (CSV, GeoJSON o .shp).",
            )
            return
        ...
        def job() -> None:
            ...
            run_classification_pipeline(
                inp, out, method=method, n_clusters=n_clusters, training_path=training
            )
    ...
    def _build_if_spectra(self) -> None:
        ...
        self.if_vector = PathRow(
            t,
            "Vector ROI",
            default="",
            directory=False,
            on_change=self._sync_if_cli,
        )
        self.if_vector.pack(fill="x", pady=(8, 2))
        vec_opts = ctk.CTkFrame(t, fg_color="transparent")
        vec_opts.pack(fill="x", pady=2)
        ctk.CTkLabel(
            vec_opts, text="Campo etiqueta", width=110, anchor="w", text_color=COLORS["muted"]
        ).pack(side="left")
        self.if_label_field = ctk.StringVar(value="")
        make_entry(vec_opts, textvariable=self.if_label_field, width=120).pack(side="left", padx=6)
        ctk.CTkButton(
            vec_opts,
            text="Extraer firmas → CSV",
            width=170,
            command=self._export_if_from_vector,
        ).pack(side="left", padx=8)
        ctk.CTkLabel(
            t,
            text="GeoJSON/SHP (puntos, líneas o polígonos). Vacío en etiqueta = solo feature_id. Mismo CRS que el cubo recomendado.",
            text_color=COLORS["muted"],
            wraplength=780,
            justify="left",
        ).pack(anchor="w", pady=(0, 4))

        panes = ctk.CTkFrame(t, fg_color="transparent", height=420)
        panes.pack(fill="x", expand=False, pady=8)
        panes.pack_propagate(False)
    ...
    def _export_if_from_vector(self) -> None:
        if self._if_cube is None:
            messagebox.showwarning("IF", "Carga un cubo IF primero.")
            return
        vector = self.if_vector.get()
        if not str(vector) or not vector.is_file():
            messagebox.showerror(
                "Vector",
                "Indica un GeoJSON o Shapefile (.shp) válido.",
            )
            return
        default = f"{self._if_cube.product_id}_roi_spectra.csv"
        path = filedialog.asksaveasfilename(
            defaultextension=".csv",
            filetypes=[("CSV", "*.csv")],
            initialfile=default,
        ...
        if not path:
            return
        label_field = self.if_label_field.get().strip() or None
        cube = self._if_cube

        def job() -> None:
            from .io_if import export_spectra_from_vector

            self.worker.report_progress("if-vector", 0.1, "Rasterizando vector…")
            ...
            self.worker.report_progress("if-vector", 1.0, f"{n} firmas exportadas")
            self.worker.log(f"Firmas IF desde vector ({n} píxeles) → {out}\n")

            def done() -> None:
                self.if_status.set(f"Vector → {n} firmas: {out.name}")
                messagebox.showinfo("CSV", f"Exportadas {n} firmas:\n{out}")
        ...
        if self.worker.busy:
            messagebox.showwarning("Ocupado", "Ya hay una tarea en ejecución.")
            return
        self.worker.run("Extraer firmas IF", job)

    # ── Pipeline ──────────────────────────────────────────────────────────

    def _build_run(self) -> None:
        t = self._tab_body("Pipeline")
        ctk.CTkLabel(
            t,
    ...
    def _build_help(self) -> None:
        ...
        grid = ctk.CTkFrame(t, fg_color=COLORS["card"], corner_radius=8)

...
def export_spectra_from_vector(
    cube: IFCube,
    vector_path: Path,
    out_path: Path,
    *,
    ...
    """
    Extrae firmas I/F de todos los píxeles cubiertos por un GeoJSON/Shapefile
    (puntos, líneas o polígonos) y las guarda en CSV ancho (una fila por píxel).

    Columnas: product_id, feature_id, label, line, sample, <wl_nm>, ...
    """
    from .spatial import pixels_under_vector

    pix = pixels_under_vector(
        cube.map_info,
        (cube.lines, cube.samples),
        vector_path,
        valid_mask=cube.valid_mask,
        label_field=label_field,
        require_label=False,
        max_pixels_per_feature=max_pixels_per_feature,
    )
    # Evitar duplicar el mismo píxel si varias geometrías se solapan
    pix = pix.drop_duplicates(subset=["line", "sample"], keep="first")

    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)

    wl_cols = [f"{float(w):.4f}" for w in cube.wavelengths]
    fieldnames = ["product_id", "feature_id", "label", "line", "sample", *wl_cols]

    with out_path.open("w", encoding="utf-8", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=fieldnames)
        writer.writeheader()
        for row in pix.itertuples(index=False):
            line = int(row.line)
            sample = int(row.sample)
            spec = cube.spectrum_at(line, sample)
            ...
            for i, col in enumerate(wl_cols):
                val = spec[i]
                out_row[col] = "" if not np.isfinite(val) else f"{float(val):.8f}"
            writer.writerow(out_row)

    return out_path, len(pix)

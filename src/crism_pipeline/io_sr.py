...
def _geotiff_array(cube: SRCube, nodata: float) -> np.ndarray:
    """Copia el cubo reemplazando rellenos CRISM (p. ej. 65535) por nodata GIS."""
    from .stretch import valid_data_mask

    data = np.array(cube.data, dtype=np.float32, copy=True)
    invalid = ~valid_data_mask(data)
    data[invalid] = np.float32(nodata)
    return data
...
def export_geotiff(cube: SRCube, out_path: Path) -> Path:
    ...
    Los píxeles de relleno CRISM (65535, etc.) se convierten al nodata del
    pipeline para que QGIS/GIS estiren solo valores válidos y muestren la imagen.
    """
    import rasterio
    ...
    nodata = float(pipeline_config()["processing"]["nodata_value"])
    data = _geotiff_array(cube, nodata)
    ...
    profile: dict = {
        ...
        "BIGTIFF": "IF_SAFER",
        "nodata": nodata,
        # GDAL gt = (origin_x, px_w, 0, origin_y, 0, -px_h); Affine(*gt) es incorrecto
